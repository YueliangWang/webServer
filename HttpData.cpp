...
/*传入一个accept之后新分配的EventLoop，以及acceptfd_。作为服务器端对客户端的处理？*/
HttpData::HttpData(EventLoop *loop, int connfd):
        loop_(loop),
        channel_(new Channel(loop, connfd)),  //new一个Channel作为loop监听
        fd_(connfd),
        error_(false),
...
void HttpData::handleRead()
{
    ...
    do
    {
        ...
        if (connectionState_ == H_DISCONNECTING)   //未连接
        {
            inBuffer_.clear();
            break;
        }
        ...
        //     break; 
        // }
        else if (zero) //读取没有失败，写端关闭zero为true
        {
		...
		//到这里表明收到有效数据
        if (state_ == STATE_PARSE_URI)  //如果state_处于解析URL状态，构造函数初始化
        {
            ...
            else if (flag == PARSE_URI_ERROR)
            {
                ...
                state_ = STATE_PARSE_HEADERS;   //前面成功状态转为解析头部
        }
        ...
        if (state_ == STATE_RECV_BODY) //POST方法需要接受body
        {
    ...
    if (!error_)    //没有错误，将前面构建的回复outBuffer_发送给客户端
    {
...
URIState HttpData::parseURI()
{
    ...
    string cop = str;
    // 读到完整的请求行再开始解析请求
    size_t pos = str.find('\r', nowReadPos_);//从nowReadPos_开始查找字符串'\r'
    if (pos < 0)
    {
        return PARSE_URI_AGAIN;
    }
    // 去掉请求行所占的空间，节省空间
    string request_line = str.substr(0, pos);  //截取第一行，请求行
    if (str.size() > pos + 1)     //if执行表明不光只有请求行
        str = str.substr(pos + 1);
    else 
        str.clear();
    // Method
    int posGet = request_line.find("GET");    //返回所在下标，没有返回-1
    ...
    // filename
    pos = request_line.find("/", pos);  //返回第一次出现‘/’的位置
    if (pos < 0)       //请求行没有目录信息
    {
    ...
    else
    {
        ...
        else
        {
            if (_pos - pos > 1)
            {
                ...
                size_t __pos = fileName_.find('?');   //?分隔实际的URL和参数
                if (__pos >= 0)
                {
    ...
    {
        if (request_line.size() - pos <= 3)  //1.1 | 1.0有三个字符，应该至少大于4
            return PARSE_URI_ERROR;
        else
        {
...
void HttpData::handleClose()
{
    ...
    shared_ptr<HttpData> guard(shared_from_this());//shared_from_this()函数返回一个当前类的std::share_ptr
    loop_->removeFromPoller(channel_);
}


void HttpData::newEvent()   //将Channel放进红黑树
{
